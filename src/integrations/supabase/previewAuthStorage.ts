...
export function brokeredPreviewStorage() {
  ...
  const scopedHost = new RegExp(String.raw`^(?:id-preview(?:-[a-z0-9]+)?|project)--(` + UUID + String.raw`)(?:-dev)?(?=\.|$)`, 'i');
  const bareHost = new RegExp('^(' + UUID + ')(?=[.-])', 'i');
  const projectId = onPreviewZone
    ? (scopedHost.exec(host)?.[1] ?? bareHost.exec(host)?.[1])
    : undefined;
  ...
  const ancestor = location.ancestorOrigins?.[0] || (document.referrer ? new URL(document.referrer).origin : '');
  const fallbackOrigins = dev ? ['https://lovable.dev', 'http://localhost:3000'] : ['https://lovable.dev'];
  const editorOrigins = ancestor && EDITOR.test(ancestor) ? [ancestor] : fallbackOrigins;
  const RESULT = 'lovable-preview-auth:result';
  const TIMEOUT = 2000;
  // CSPRNG: os requestIds correlacionam respostas do broker; nunca usar Math.random.
