...
export function brokeredPreviewStorage() {
  ...
  const namedHostRe = new RegExp(String.raw`^(?:id-preview(?:-[a-z0-9]+)?|project)--(` + UUID + String.raw`)(?:-dev)?(?=\.|$)`, 'i');
  const bareHostRe = new RegExp('^(' + UUID + ')(?=[.-])', 'i');
  const projectId = onPreviewZone
    ? (namedHostRe.exec(host)?.[1] ?? bareHostRe.exec(host)?.[1])
    : undefined;
  ...
  const devFallbackOrigins = dev ? ['https://lovable.dev', 'http://localhost:3000'] : ['https://lovable.dev'];
  const editorOrigins = ancestor && EDITOR.test(ancestor) ? [ancestor] : devFallbackOrigins;
