    ...
    <div className="w-full min-w-0 max-w-6xl mx-auto overflow-x-hidden px-3 py-4 sm:p-8">
      <header className="mb-6 flex items-end justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap w-full sm:w-auto">
          {!open && <Button className="flex-1 sm:flex-none" onClick={() => setOpenDialog(true)}><Wallet className="h-4 w-4 mr-1"/>Abrir caixa</Button>}
          {open && (
            <>
              <Button size="sm" variant="outline" className="flex-1 sm:flex-none" onClick={() => setMovDialog('suprimento')}><ArrowDownCircle className="h-4 w-4 mr-1"/>Suprimento</Button>
              <Button size="sm" variant="outline" className="flex-1 sm:flex-none" onClick={() => setMovDialog('sangria')}><ArrowUpCircle className="h-4 w-4 mr-1"/>Sangria</Button>
              <Button size="sm" onClick={() => setCloseDialog(true)} className="bg-primary flex-1 sm:flex-none"><Lock className="h-4 w-4 mr-1"/>Fechar</Button>
            </>
          )}
        </div>
      </header>
